...
function get_sleeptime_data_of_date($dbconn,$user_id,$date){
  // 일별로, 30일치
  $res=array("success"=>false,"result"=>array(),"min_max_avg"=>array());
  
  try{
    //GET AVERAGE
    $sql = "SELECT CAST(MIN(sleep_time) as UNSIGNED) as min,CAST(MAX(sleep_time) as UNSIGNED) as max,CAST(AVG(sleep_time) as UNSIGNED) as average FROM day_profile WHERE user_id = '$user_id' AND date = '$date';";
    $result = mysqli_query($dbconn,$sql);
    $row = mysqli_fetch_assoc($result);
    $res['min_max_avg']= $row;
    
    //GET ALL RESULTS
    $sql = "SELECT profile_id,date,sleep_time FROM day_profile WHERE user_id = '$user_id' AND date = '$date';";
    $result = mysqli_query($dbconn,$sql);
    for($i=0;$i<$result->num_rows;$i++){
      $row = mysqli_fetch_assoc($result);
      array_push($res['result'], $row);
